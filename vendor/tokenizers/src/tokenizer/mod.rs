...
pub trait Trainer {
    ...
    /// Tell the trainer about the tokens the tokenizer holds *outside* of the model, and the
    /// ids it has given them.
    ///
    /// The tokenizer calls this just before training. It matters for trainers that keep the
    /// model's existing ids: added tokens live outside the model but share its id space, so a
    /// trainer that appends to the vocabulary would otherwise hand a new token an id an added
    /// token already owns. The default implementation does nothing, which is correct for any
    /// trainer that renumbers the whole vocabulary anyway.
    fn reserve_tokens(&mut self, _tokens: &HashMap<String, u32>) {}
}
        ...
        T: Trainer<Model = M> + Sync,
    {
        ...
        ResultShunt::process(
            ...
            |sequences| -> Result<()> {
                ...
                trainer.reserve_tokens(self.added_vocabulary.get_vocab());
