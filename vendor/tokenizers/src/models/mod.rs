...
impl Trainer for TrainerWrapper {
    ...
    fn reserve_tokens(&mut self, tokens: &HashMap<String, u32>) {
        match self {
            Self::BpeTrainer(bpe) => bpe.reserve_tokens(tokens),
            Self::WordPieceTrainer(wpt) => wpt.reserve_tokens(tokens),
            Self::WordLevelTrainer(wpt) => wpt.reserve_tokens(tokens),
            Self::UnigramTrainer(wpt) => wpt.reserve_tokens(tokens),
        }
    }
}
