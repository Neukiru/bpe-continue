...
impl Word {
    ...
    /// Iterate over every pair of adjacent symbols, along with the length of the token that
    /// merging the pair would produce.
    pub(super) fn get_pairs_iter(&self) -> impl Iterator<Item = (Pair, usize)> + '_ {
        self.symbols
            .windows(2)
            .map(|w| ((w[0].c, w[1].c), w[0].len + w[1].len))
    }
